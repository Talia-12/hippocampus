...
async fn test_record_review_edge_cases() {
	...
	assert!(
		result
			.unwrap_err()
			.to_string()
			.contains("Rating must be between 1 and 4")
	);
	...
	assert!(
		data1["stability"].as_f64().is_some(),
		"Should have stability"
	);
	assert!(
		data1["difficulty"].as_f64().is_some(),
	...
	assert!(
		data2["stability"].as_f64().unwrap() > 0.0,
		"Stability should be positive"
	);
	assert!(
		data2["difficulty"].as_f64().unwrap() > 0.0,
	...
	assert!(
		data3["stability"].as_f64().is_some(),
		"Should have stability after multiple reviews"
	);
	assert!(
		data3["difficulty"].as_f64().is_some(),
		"Should have difficulty after multiple reviews"
	);
}
...
async fn test_migrate_scheduler_data() {
	...
	assert!(
		data["stability"].as_f64().is_some(),
		"Should have stability after migration"
	);
	assert!(
		data["difficulty"].as_f64().is_some(),
		"Should have difficulty after migration"
	);
	assert!(
		data.get("ease_factor").is_none(),
		"Should not have ease_factor after migration"
	);
...
async fn test_record_review_incremental_queue() {
	...
	let item_type = create_item_type(
		&pool,
		"IQ Test Type".to_string(),
		"incremental_queue".to_string(),
	)
	.await
	.unwrap();
...
async fn test_record_review_review_function_lookup_fails() {
	...
	assert!(
		result
			.unwrap_err()
			.to_string()
			.contains("Failed to look up review function"),
...
fn test_calculate_next_review_unknown_function() {
	...
	assert!(
		result
			.unwrap_err()
			.to_string()
			.contains("Unknown review function: unknown_function"),
...
async fn test_migrate_none_fsrs_0_memory_state_from_sm2_err() {
	...
	assert!(
		result.is_err(),
		"Migration should fail when memory_state_from_sm2 returns Err"
	);
}
...
async fn test_migrate_none_fsrs_0_scheduler_data_none_skipped() {
	...
	let card_before = crate::schema::cards::table
		...
		.unwrap();
	assert!(
		card_before.get_scheduler_data().is_none(),
		"Precondition: scheduler_data is None"
	);
	...
	let card_after = crate::schema::cards::table
		...
		.unwrap();
	assert!(
		card_after.get_scheduler_data().is_none(),
		"Card with None scheduler_data should be unchanged"
	);
}
...
async fn test_migrate_none_fsrs_0_scheduler_data_not_object_skipped() {
	...
	assert!(
		data.0.is_array(),
		"Non-object scheduler_data should be left unchanged"
	);
}
...
async fn test_migrate_fsrs_0_fsrs_1_with_iq_types() {
	...
	let fsrs_data = |stability: f64| {
		Some(JsonValue(
			json!({"stability": stability, "difficulty": 5.0}),
		))
	};
	...
	let updated_todo_type = crate::schema::item_types::table
		...
		.unwrap();
	assert_eq!(
		updated_todo_type.get_review_function(),
		"incremental_queue",
	...
	let updated_ir_type = crate::schema::item_types::table
		...
		.unwrap();
	assert_eq!(
		updated_ir_type.get_review_function(),
		"incremental_queue",
	...
	let unchanged_fsrs_type = crate::schema::item_types::table
		...
		.unwrap();
	assert_eq!(
		unchanged_fsrs_type.get_review_function(),
		"fsrs",
	...
	assert_eq!(
		todo_data["interval"].as_f64().unwrap(),
		15.0,
	...
	assert_eq!(
		ir_data["interval"].as_f64().unwrap(),
		30.0,
