...
async fn test_list_tags() {
	...
	let tag1 = create_tag(&pool, "Important".to_string(), true)
		.await
		.unwrap();
	let tag2 = create_tag(&pool, "Difficult".to_string(), false)
		.await
		.unwrap();
...
async fn test_list_tags_for_card() {
	...
	add_tag_to_item(&pool, &tag1.get_id(), &item.get_id())
		.await
		.unwrap();
	add_tag_to_item(&pool, &tag2.get_id(), &item.get_id())
		.await
		.unwrap();
...
async fn test_add_tag_to_item() {
	...
	add_tag_to_item(&pool, &tag.get_id(), &item.get_id())
		.await
		.unwrap();
...
async fn test_remove_tag_from_item() {
	...
	remove_tag_from_item(&pool, &tag.get_id(), &item.get_id())
		.await
		.unwrap();
