...
pub async fn record_review(pool: &DbPool, card_id: &CardId, rating_val: i32) -> Result<Review> {
	...
	debug!(
		"Calculating next review date using review function: {}",
		review_function
	);
	...
	info!(
		"Successfully recorded review with id: {}",
		new_review.get_id()
	);
...
fn calculate_next_review(
	...
	review_function: &str,
	rating: i32,
) -> Result<(chrono::DateTime<Utc>, JsonValue)> {
...
fn calculate_next_fsrs_review(
	...
	debug!(
		"Current state: {:?}, days_elapsed: {}",
		current_state, days_elapsed
	);
	...
	debug!(
		"Next review scheduled for: {}, stability: {}, difficulty: {}",
...
/// A Result containing a tuple of (next_review, scheduler_data)
#[instrument(skip_all, fields(card_id = %card.get_id(), rating = %rating))]
fn calculate_next_incremental_queue_review(
	card: &Card,
	rating: i32,
) -> Result<(chrono::DateTime<Utc>, JsonValue)> {
	...
	let current_interval = data.get("interval").and_then(|v| v.as_f64()).unwrap_or(1.0);
...
pub async fn get_all_next_reviews_for_card(
	...
	debug!(
		"Found card, calculating next reviews for all possible ratings using {}",
		review_function
	);
	...
	for rating in 1..=4 {
		...
		match calculate_next_review(&card, &review_function, rating) {
			...
			Err(e) => {
				warn!(
					"Failed to calculate next review for rating {}: {}",
					rating, e
				);
				return Err(anyhow!(
					"Failed to calculate next review for rating {}: {}",
	...
	info!(
		"Successfully calculated {} possible next reviews for card {}",
		results.len(),
		card_id
	);
...
fn migrate_scheduler_data_none_fsrs_0(conn: &mut diesel::SqliteConnection) -> Result<()> {
		...
		if let Some(data) = card.get_scheduler_data() {
			if let Some(obj) = data.0.as_object() {
				...
				if let (Some(ease), Some(interval)) = (
					...
					let memory = fsrs.memory_state_from_sm2(ease as f32, interval as f32, 0.9)?;
	...
	info!(
		"Successfully migrated {} cards from SM-2 to FSRS",
		migrated_count
	);
	Ok(())
}
...
fn migrate_scheduler_data_fsrs_0_fsrs_1(conn: &mut diesel::SqliteConnection) -> Result<()> {
	...
	info!(
		"Set review_function to 'incremental_queue' for {} item types",
		iq_type_ids.len()
	);
	...
	info!(
		"Converted {} cards to incremental queue format",
		converted_count
	);
	Ok(())
}
