...
fn resolve_relation_filter(pool: &DbPool, query: &GetQueryDto) -> Result<Option<HashSet<String>>> {
    match (&query.parent_item_id, &query.child_item_id) {
        ...
        (Some(parent_id), Some(child_id)) => {
            let children: HashSet<String> = super::item_relation_repo::get_children_of(pool, parent_id)?.into_iter().collect();
            let parents: HashSet<String> = super::item_relation_repo::get_parents_of(pool, child_id)?.into_iter().collect();
