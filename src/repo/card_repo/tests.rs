...
async fn test_create_card() {
	...
	// Create an item type
	let item_type = create_item_type(&pool, "Test Type 1".to_string(), "fsrs".to_string())
		.await
		.unwrap();
	...
	let card_index = 2;
	let priority = 0.3;
	let card = create_card(&pool, &item.get_id(), card_index, priority)
		.await
		.unwrap();
...
async fn test_list_all_cards() {
	...
	assert!(
		cards
			.iter()
			.filter(|c| c.get_item_id() == item1.get_id())
			.count() == 2
	);
	assert!(
		cards
			.iter()
			.filter(|c| c.get_item_id() == item2.get_id())
			.count() == 2
	);
}
...
async fn test_filter_cards_by_item_type() {
	...
	let type1_type = create_item_type(&pool, "Test Type 1".to_string(), "fsrs".to_string())
		.await
		.unwrap();
	let type2_type = create_item_type(&pool, "Test Type 2".to_string(), "fsrs".to_string())
		.await
		.unwrap();
...
async fn test_filter_cards_by_tags() {
	...
	// Create some tags
	let tag1 = create_tag(&pool, "Important".to_string(), true)
		.await
		.unwrap();
	let tag2 = create_tag(&pool, "Difficult".to_string(), true)
		.await
		.unwrap();

	// Add tags to items
	add_tag_to_item(&pool, &tag1.get_id(), &item1.get_id())
		.await
		.unwrap();
	add_tag_to_item(&pool, &tag2.get_id(), &item1.get_id())
		.await
		.unwrap();
	add_tag_to_item(&pool, &tag1.get_id(), &item2.get_id())
		.await
		.unwrap();
	// Item 3 has no tags

	// Filter cards by tags
	let query1 = GetQueryDtoBuilder::new().add_tag_id(tag1.get_id()).build();

	let query2 = GetQueryDtoBuilder::new().add_tag_id(tag2.get_id()).build();
	...
	assert!(
		cards_both_tags
			.iter()
			.all(|c| c.get_item_id() == item1.get_id())
	);
}
...
async fn test_filter_cards_with_multiple_criteria() {
	...
	// Create some tags
	let important_tag = create_tag(&pool, "Important".to_string(), true)
		.await
		.unwrap();

	// Add tags to items
	add_tag_to_item(&pool, &important_tag.get_id(), &type1_item1.get_id())
		.await
		.unwrap();
	add_tag_to_item(&pool, &important_tag.get_id(), &type2_item.get_id())
		.await
		.unwrap();
	...
	assert!(
		filtered_cards
			.iter()
			.any(|c| c.get_id() == type1_card1.get_id())
	);
...
async fn test_update_card_priority() {
	...
	let card = create_card(&pool, &item.get_id(), 2, initial_priority)
		.await
		.unwrap();
...
async fn test_filter_cards_by_suspended_state_exclude() {
	...
	assert!(
		filtered_cards
			.iter()
			.any(|c| c.get_id() == cards[1].get_id())
	);
	assert!(
		!filtered_cards
			.iter()
			.any(|c| c.get_id() == cards[0].get_id())
	);
}
...
async fn test_filter_cards_complex_query() {
	...
	// Create a tag
	let tag = create_tag(&pool, "Important".to_string(), true)
		.await
		.unwrap();

	// Add tag to the item
	add_tag_to_item(&pool, &tag.get_id(), &item1.get_id())
		.await
		.unwrap();
...
async fn test_clear_sort_positions_with_item_type_filter() {
	...
	let type_a = create_item_type(&pool, "Test Type A".to_string(), "fsrs".to_string())
		.await
		.unwrap();
	let type_b = create_item_type(&pool, "Test Type B".to_string(), "fsrs".to_string())
		.await
		.unwrap();
...
async fn test_clear_sort_positions_with_tag_filter() {
	...
	for (id, orig_pos) in &item2_before {
		...
		assert_eq!(
			...
			orig_pos,
			"untagged card {} should be unchanged",
			id
		);
	}
}
...
async fn test_clear_sort_positions_default_query_clears_all() {
	...
	clear_sort_positions(&pool, &GetQueryDto::default())
		.await
		.unwrap();
...
async fn test_clear_sort_positions_mixed_sort_positions() {
	...
	for c in &cards_b_after {
		...
		assert_eq!(
			c.get_sort_position(),
			orig,
			"type B card {} should be unchanged",
			c.get_id()
		);
	}
}
...
async fn test_list_cards_with_parent_item_id_filter() {
    ...
    let parent = crate::repo::create_item(&pool, &item_type.get_id(), "Parent".to_string(), serde_json::json!({"front": "F", "back": "B"})).await.unwrap();
    let child1 = crate::repo::create_item(&pool, &item_type.get_id(), "Child 1".to_string(), serde_json::json!({"front": "F", "back": "B"})).await.unwrap();
    let child2 = crate::repo::create_item(&pool, &item_type.get_id(), "Child 2".to_string(), serde_json::json!({"front": "F", "back": "B"})).await.unwrap();
    let _unrelated = crate::repo::create_item(&pool, &item_type.get_id(), "Unrelated".to_string(), serde_json::json!({"front": "F", "back": "B"})).await.unwrap();
...
async fn test_list_cards_with_child_item_id_filter() {
    ...
    let parent1 = crate::repo::create_item(&pool, &item_type.get_id(), "Parent 1".to_string(), serde_json::json!({"front": "F", "back": "B"})).await.unwrap();
    let parent2 = crate::repo::create_item(&pool, &item_type.get_id(), "Parent 2".to_string(), serde_json::json!({"front": "F", "back": "B"})).await.unwrap();
    let child = crate::repo::create_item(&pool, &item_type.get_id(), "Child".to_string(), serde_json::json!({"front": "F", "back": "B"})).await.unwrap();
    let _unrelated = crate::repo::create_item(&pool, &item_type.get_id(), "Unrelated".to_string(), serde_json::json!({"front": "F", "back": "B"})).await.unwrap();
