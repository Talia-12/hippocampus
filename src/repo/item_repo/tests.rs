...
async fn test_create_item() {
	...
	let item = create_item(&pool, &item_type.get_id(), title.clone(), data.clone())
		.await
		.unwrap();
...
async fn test_get_item() {
	...
	let created_item = create_item(&pool, &item_type.get_id(), title.clone(), data.clone())
		.await
		.unwrap();
...
async fn test_get_items_by_type() {
	...
	let vocab_type = create_item_type(&pool, "Test Type 1".to_string(), "fsrs".to_string())
		.await
		.unwrap();
	let grammar_type = create_item_type(&pool, "Test Type 2".to_string(), "fsrs".to_string())
		.await
		.unwrap();
...
async fn test_update_item_title() {
	...
	let updated_item = update_item(&pool, &created_item.get_id(), Some(new_title.clone()), None)
		.await
		.unwrap();
...
async fn test_update_item_data() {
	...
	let updated_item = update_item(&pool, &created_item.get_id(), None, Some(new_data.clone()))
		.await
		.unwrap();
...
async fn test_list_items_with_filters_item_type_only() {
	...
	let type1 = create_item_type(&pool, "Test Type A".to_string(), "fsrs".to_string())
		.await
		.unwrap();
	let type2 = create_item_type(&pool, "Test Type B".to_string(), "fsrs".to_string())
		.await
		.unwrap();
...
async fn test_list_items_with_filters_suspended_only() {
	...
	for card in &cards {
		crate::repo::set_card_suspended(&pool, &card.get_id(), true)
			.await
			.unwrap();
	}
...
async fn test_list_items_with_filters_by_tag() {
	...
	let tag = crate::repo::create_tag(&pool, "Special".to_string(), true)
		.await
		.unwrap();
	crate::repo::add_tag_to_item(&pool, &tag.get_id(), &item1.get_id())
		.await
		.unwrap();
...
async fn test_list_items_with_filters_deduplicates_across_cards() {
	...
	assert_eq!(
		items.iter().filter(|i| i.get_id() == item.get_id()).count(),
		1
	);
}
...
async fn test_update_with_empty_changes() {
	...
	let updated_item = update_item(&pool, &created_item.get_id(), None, None)
		.await
		.unwrap();
...
async fn test_create_item_unknown_item_type_name_fails() {
	...
	let item_type = create_item_type(&pool, "0".to_string(), "fsrs".to_string())
		.await
		.unwrap();
	...
	assert!(
		result.is_err(),
		"create_item should fail for unknown item type name"
	);
	let err = result.unwrap_err().to_string();
	assert!(
		err.contains("Unable to construct cards for unknown item type"),
		"Error should mention unknown item type, got: {}",
		err
	);
}
