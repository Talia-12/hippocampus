...
async fn test_create_item_type() {
	...
	let item_type = create_item_type(&pool, name.clone(), "fsrs".to_string())
		.await
		.unwrap();
...
async fn test_create_item_type_incremental_queue() {
	...
	let item_type = create_item_type(&pool, name.clone(), "incremental_queue".to_string())
		.await
		.unwrap();
...
async fn test_get_item_type() {
	...
	let created_item_type = create_item_type(&pool, name.clone(), "fsrs".to_string())
		.await
		.unwrap();
	let retrieved_item_type = get_item_type(&pool, &created_item_type.get_id())
		.unwrap()
		.unwrap();
...
async fn test_list_item_types() {
	...
	let item_type1 = create_item_type(&pool, "Type 1".to_string(), "fsrs".to_string())
		.await
		.unwrap();
	let item_type2 = create_item_type(&pool, "Type 2".to_string(), "fsrs".to_string())
		.await
		.unwrap();
...
async fn test_update_item_type_review_function() {
	...
	let item_type = create_item_type(&pool, "Todo".to_string(), "fsrs".to_string())
		.await
		.unwrap();
	assert_eq!(item_type.get_review_function(), "fsrs");

	let updated = update_item_type_review_function(
		&pool,
		&item_type.get_id(),
