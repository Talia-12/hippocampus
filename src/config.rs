...
impl Config {
	...
	pub fn apply_update(self, update: ConfigUpdate) -> Self {
		Self {
			...
			backup_interval_minutes: update
				.backup_interval_minutes
				.unwrap_or(self.backup_interval_minutes),
...
pub fn base_config(data_dir_path: Option<PathBuf>) -> Config {
	let database_url = data_dir_path.map_or("srs_server.db".to_string(), |path| {
		path.join("srs_server.db").to_string_lossy().to_string()
	});
