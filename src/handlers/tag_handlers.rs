...
pub async fn list_tags_handler(
	...
	let tags = repo::list_tags(&pool).map_err(ApiError::Database)?;
...
pub async fn list_tags_for_item_handler(
	...
	let tags = repo::list_tags_for_item(&pool, &item_id).map_err(ApiError::Database)?;
...
mod tests {
	...
	async fn test_create_tag_handler() {
		...
		// Call the handler
		let result = create_tag_handler(State(pool.clone()), Json(payload))
			.await
			.unwrap();
	...
	async fn test_list_tags_handler() {
		...
		// Create some tags
		let tag1 = repo::create_tag(&pool, "Important".to_string(), true)
			.await
			.unwrap();
		let tag2 = repo::create_tag(&pool, "Difficult".to_string(), false)
			.await
			.unwrap();

		// Call the handler
		let result = list_tags_handler(State(pool.clone())).await.unwrap();
	...
	async fn test_list_tags_for_item_handler() {
		...
		repo::add_tag_to_item(&pool, &tag1.get_id(), &item.get_id())
			.await
			.unwrap();
		repo::add_tag_to_item(&pool, &tag2.get_id(), &item.get_id())
			.await
			.unwrap();

		// Call the handler
		let result = list_tags_for_item_handler(State(pool.clone()), Path(item.get_id()))
			.await
			.unwrap();
	...
	async fn test_list_tags_for_card_handler() {
		...
		// Call the handler
		let result = list_tags_for_card_handler(State(pool.clone()), Path(card.get_id()))
			.await
			.unwrap();
