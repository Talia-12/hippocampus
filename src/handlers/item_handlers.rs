...
pub async fn get_item_handler(
	...
	let item = repo::get_item(&pool, &item_id).map_err(ApiError::Database)?;
...
pub async fn update_item_handler(
	...
	let updated_item = repo::update_item(&pool, &item_id, payload.title, payload.item_data)
		.await
		.map_err(ApiError::Database)?;

	info!(
		"Successfully updated item with id: {}",
		updated_item.get_id()
	);
...
pub async fn list_items_handler(
	...
	let items = repo::list_items_with_filters(&pool, &query).map_err(ApiError::Database)?;
...
pub async fn list_items_by_item_type_handler(
	...
	let items = repo::get_items_by_type(&pool, &item_type.get_id()).map_err(ApiError::Database)?;

	info!(
		"Retrieved {} items for item type {}",
		items.len(),
		item_type_id
	);
...
mod tests {
	...
	async fn test_create_item_handler() {
		...
		// Call the handler
		let result = create_item_handler(State(pool.clone()), Json(payload))
			.await
			.unwrap();
	...
	async fn test_list_items_handler() {
		...
		// Call the handler
		let result = list_items_handler(State(pool.clone()), Query(GetQueryDto::default()))
			.await
			.unwrap();
	...
	async fn test_list_items_handler_with_item_type_filter() {
		...
		let type1 = repo::create_item_type(&pool, "Test Type A".to_string(), "fsrs".to_string())
			.await
			.unwrap();
		let type2 = repo::create_item_type(&pool, "Test Type B".to_string(), "fsrs".to_string())
			.await
			.unwrap();
		...
		let result = list_items_handler(State(pool.clone()), Query(query))
			.await
			.unwrap();
	...
	async fn test_list_items_handler_with_suspended_filter() {
		...
		for card in &cards {
			repo::set_card_suspended(&pool, &card.get_id(), true)
				.await
				.unwrap();
		}
	...
	async fn test_get_item_handler() {
		...
		// Call the handler
		let result = get_item_handler(State(pool.clone()), Path(item.get_id()))
			.await
			.unwrap();
	...
	async fn test_list_items_by_item_type_handler() {
		...
		let type1 = repo::create_item_type(&pool, "Test Type 1".to_string(), "fsrs".to_string())
			.await
			.unwrap();
		let type2 = repo::create_item_type(&pool, "Test Type 2".to_string(), "fsrs".to_string())
			.await
			.unwrap();
		...
		let result = list_items_by_item_type_handler(State(pool.clone()), Path(type1.get_id()))
			.await
			.unwrap();
	...
	async fn test_delete_item_handler_success() {
		...
		let result = delete_item_handler(State(pool.clone()), Path(item.get_id())).await;
	...
	async fn test_delete_item_deletes_associated_cards() {
		...
		assert!(
			...
			"Item should have associated cards"
		);

		// Call the handler to delete the item
		let _ = delete_item_handler(State(pool.clone()), Path(item.get_id()))
			.await
			.unwrap();
		...
		assert!(
			cards_result.is_err(),
			"Should get an error when trying to fetch cards for deleted item"
		);
		assert!(
			cards_result.unwrap_err().to_string().contains("not found"),
	...
	async fn test_delete_item_deletes_associated_reviews() {
		...
		assert_eq!(
			reviews_before.len(),
			2,
			"Should have 2 reviews for the card"
		);
		assert!(
			reviews_before
				.iter()
				.any(|r| r.get_id() == review1.get_id()),
			"First review should exist"
		);
		assert!(
			reviews_before
				.iter()
				.any(|r| r.get_id() == review2.get_id()),
			"Second review should exist"
		);
		...
		assert_eq!(
			review_count, 0,
			"All reviews for the card should be deleted when the item is deleted"
		);
	}
	...
	async fn test_delete_item_with_tags() {
		...
		// Create a tag
		let tag = repo::create_tag(&pool, "Important".to_string(), true)
			.await
			.unwrap();

		// Add the tag to the item
		repo::add_tag_to_item(&pool, &tag.get_id(), &item.get_id())
			.await
			.unwrap();
		...
		assert_eq!(
			tags_before[0].get_id(),
			tag.get_id(),
			"Tag should be associated with item"
		);
		...
		assert_eq!(
			tag_count, 0,
			"Tag associations should be deleted when the item is deleted"
		);
		...
		assert!(
			tag_after.is_ok(),
			"The tag itself should not be deleted: {:?}",
			tag_after
		);
	}
}
