...
fn test_new_sets_fields() {
	let item_id = ItemId("item-123".to_string());
	let tag_id = TagId("tag-456".to_string());
...
fn test_created_at_is_recent() {
	let before = Utc::now();
	let item_tag = ItemTag::new(ItemId("item".to_string()), TagId("tag".to_string()));
	let after = Utc::now();

	let created_at = item_tag.get_created_at();
	assert!(
		created_at >= before,
		"created_at should be >= test start time"
	);
