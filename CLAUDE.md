...
- **Tune `PROPTEST_CASES` to the phase of work**: During development, run `PROPTEST_CASES=1 <test_cmd>` for the fastest feedback loop, or `PROPTEST_CASES=5 <test_cmd>` for a bit more confidence. Before declaring a task done, run `<test_cmd>` with the full default (256 cases).
