...
async fn create_review(app: &mut axum::Router, card_id: &CardId, rating: i32) -> Value {
	...
	assert_eq!(
		parts.status,
		StatusCode::OK,
		"Expected 200 OK status, instead got {}: {}",
		parts.status,
...
async fn test_create_review_rating_again() {
	...
	let next_review =
		chrono::DateTime::parse_from_rfc3339(&format!("{}Z", next_review_str)).unwrap();

	// Get the last review date
	let last_review_str = updated_card["last_review"].as_str().unwrap();
	let last_review =
		chrono::DateTime::parse_from_rfc3339(&format!("{}Z", last_review_str)).unwrap();
	...
	assert!(
		days >= 0.0 && days <= 2.0,
		"Expected next review within 2 days for 'Again', but was {} days",
		days
	);
}
...
async fn test_create_review_rating_hard() {
	...
	assert!(
		days >= 0.5 && days <= 10.0,
		"Expected next review within a reasonable range for 'Hard', but was {} days",
		days
	);
}
...
async fn test_create_review_rating_good() {
	...
	assert!(
		days >= 0.5 && days <= 30.0,
		"Expected next review within a reasonable range for 'Good', but was {} days",
		days
	);
}
...
async fn test_create_review_rating_easy() {
	...
	assert!(
		days >= 1.0 && days <= 60.0,
		"Expected next review within a reasonable range for 'Easy', but was {} days",
		days
	);
}
...
async fn test_create_multiple_reviews() {
	...
	assert!(
		days >= 0.5,
		"Expected next review to be at least half a day after second review, but was {} days",
		days
	);
	...
	assert!(
		stability > 0.0,
		"Expected positive stability after third review, but was {}",
		stability
	);
	assert!(
		difficulty > 0.0 && difficulty <= 10.0,
		"Expected difficulty in valid range after third review, but was {}",
		difficulty
	);
}
...
async fn test_list_reviews_for_card() {
	...
	assert!(
		reviews.len() >= 2,
		"Expected at least 2 reviews, got {}",
		reviews.len()
	);

	// Check that our reviews are in the list
	let has_review1 = reviews
		.iter()
		.any(|r| r["id"].as_str().unwrap() == review1["id"].as_str().unwrap());
	let has_review2 = reviews
		.iter()
		.any(|r| r["id"].as_str().unwrap() == review2["id"].as_str().unwrap());
